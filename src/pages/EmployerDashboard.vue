...
import HeroImage from '../assets/emp1.png'
...
function handleUpdate(updatedJob) {
    ...
    if (index !== -1) {
        ...
        localJobs.value.push(updatedJob)
    }
    localStorage.setItem('jobs', JSON.stringify(localJobs.value))
...
<template>
    <div class="p-4">
        <Hero :image="HeroImage" :showButton="false" :typedStrings="['next hire today.', 'ideal candidate easily.', 'job post in minutes.']" />
        ...
        <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 px-2">
            <div
                ...
                <h2 class="text-xl font-bold mb-4 text-start">Edit Job</h2>
